...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const res = await getInspectionHistoryService();
      if (res.data.success) {
        // Backend usually returns 'user' or 'data' array
      ...
      // Optional: if (err.response?.status === 401) router.push("/login");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteInspectionRecordService(id);
      if (res.data.success) {
  ...
  // 🔹 VIEW RESULT
  const handleView = (inspection) => {
    // Navigate using the backend ID
    router.push(`/result?id=${inspection._id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 px-4 py-12">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Filters and Search (UI Restored) */}
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          <div className="grid md:grid-cols-2 gap-4 mb-6">
            {/* Search */}
            <div className="relative">
        ...
        {/* Inspections List (UI Restored) */}
        {filteredInspections.length > 0 ? (
          <div className="space-y-4">
            {filteredInspections.map((inspection) => (
              <div key={inspection._id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition p-6">
                <div className="flex flex-col md:flex-row gap-6">
                  {/* Image Thumbnail */}
                  <div className="md:w-40 flex-shrink-0">
                    <img
                      src={inspection.imageUrl} // Updated to backend field name
                      alt="Vehicle"
                      className="w-full h-32 object-cover rounded-lg"
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="text-sm text-slate-600 mb-1">Damage Type</p>
                        <p className="text-lg font-bold text-slate-900 capitalize">{inspection.damageType}</p>
                      </div>
                      <div>
                        <p className="text-sm text-slate-600 mb-1">Location</p>
                        <p className="text-lg font-bold text-slate-900">{inspection.location}</p>
